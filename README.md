<div align="center">
  <img src="docs/logo-animated.svg" width="180" alt="ManifestDexCore logo">

  <h1>ManifestDexCore</h1>
  ...
  <p>
    ...
    <a href="https://deepwiki.com/OpenSteam001/ManifestDexCore">
...
- Support auto manifest download via `manifestdexcore` / `steamrun` / `wudrm` upstream APIs (default is `manifestdexcore`), or a custom Lua endpoint (see [Manifest via Lua](#manifest-via-lua)).
...
### Injection
- Add optional game-process library injection through `[inject]` in `manifestdexcore.toml`.
...
- Bypass Steam Family Sharing restrictions for games that have been added to the library with `addappid` in Lua. All accounts in the Steam Family that participate in sharing must use ManifestDexCore for this to work.

### Compatible with games protected by Denuvo and SteamStub
- SteamStub-only games do not require configuring `AppTicket`. ManifestDexCore can reuse Steam's local ConfigStore ticket and forge the requested AppId through a SteamDRMP off-by-four ticket parsing vulnerability, without injecting into the game process.
- Denuvo-protected games still require explicit ticket data. ManifestDexCore stores `AppTicket` and `ETicket` through the platform credential store.
...
- AppTicket priority: explicit tickets have the highest priority, including tickets configured by `setAppTicket` and existing cached `AppTicket` credential values. If no explicit AppTicket is available, ManifestDexCore falls back to the forged local ConfigStore ticket path.
...
- If no `setStat` is configured for an app, ManifestDexCore queries `https://stats.opensteamtool.com/{appid}` when `[stats] enable_api = true` (default).
...
- Add `-launchexe="<path>"` to the Steam launch parameters to launch a game with a different executable than its default one (for example, a modded or patched exe). The `-launchexe` token is stripped before the game starts, and any remaining launch parameters are passed through untouched:
  ```
  -launchexe="C:\Program Files (x86)\Steam\steamapps\common\Assassin's Creed Black Flag Resynced\ACBlackFlag.exe" -windowed
  ```
...
2. Copy generated `dwmapi.dll`, `xinput1_4.dll` and `ManifestDexCore.dll` to the Steam root directory.
3. Create Lua directory (for example `C:\steam\config\stplug-in`) and place Lua scripts there. The DLL will automatically load and execute them.
4. Lua example:
```lua
...
Rename `manifestdexcore.example.toml` to `manifestdexcore.toml` and place it in the Steam root directory (next to `steam.exe`).
...
[manifest]
# Upstream API for depot manifest request codes.  Options: "manifestdexcore", "steamrun", "wudrm"
url = "manifestdexcore"
...
enabled = false
# library_x64 = "ManifestDexCore.GameHook.x64.dll"
# library_x86 = "ManifestDexCore.GameHook.x86.dll"
...
ManifestDexCore no longer ships byte-pattern signatures inside the DLL. Instead, on each launch it computes the SHA-256 of `steamclient64.dll` and `steamui.dll` on disk and looks up a matching pattern file from the upstream tracker at [`OpenSteam001/steam-monitor`](https://github.com/OpenSteam001/steam-monitor) (`pattern` branch).
...
3. **Local cache** — `<Steam>\manifestdexcore\pattern\<subdir>\<sha256>.toml`. Used **only** when remote is unreachable. The cache is overwritten after every successful remote fetch.
...
[remote]
url_template = "https://your.server/{channel}/{component}/{sha256}.toml"
# url_template = "https://fastly.jsdelivr.net/gh/OpenSteam001/steam-monitor@{channel}/{component}/{sha256}.toml"
```

### Debug logging

Debug builds write per-module log files under `<Steam>/manifestdexcore/`:
...
The log level is controlled by `[log] level` in `manifestdexcore.toml`.
