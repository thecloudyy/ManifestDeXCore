...
namespace {
    ...
    struct Snapshot {
        std::string manifestProvider = "manifestdexcore";
    ...
    Snapshot MakeDefaultSnapshot(const std::string& configPath) {
        ...
        snapshot.logDir = (p.parent_path() / "manifestdexcore").string();
        return snapshot;
    }
    ...
    void ApplySnapshot(const Snapshot& snapshot) {
        ...
        mdxBrowserEnabled      = snapshot.mdxBrowser.enabled;
    }
    ...
    void ApplyManifestProvider(const std::string& provider) {
        if (!ManifestClient::SetProvider(provider)) {
            ...
            ManifestClient::SetProvider("manifestdexcore");
        }
    }
    ...
    LoadResult Load(const std::string& configPath) {
        ...
        try {
            ...
            // [mdx_browser]
            if (auto mdxBrowser = tbl["mdx_browser"].as_table()) {
                if (auto val = (*mdxBrowser)["enabled"].value<bool>())
                    snapshot.mdxBrowser.enabled = *val;
            }
