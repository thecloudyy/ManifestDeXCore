...
#include "ManifestDexCoreBuildInfo.h"
...
namespace {
    ...
    struct Snapshot {
        std::string manifestDexCoreVersion = MANIFESTDEXCORE_VERSION;
    ...
    static std::string AppendSnapshot(std::string message)
    {
        message +=
            "\n\nSteam diagnostics:\n"
            "  ManifestDexCore version: " + g_snapshot.manifestDexCoreVersion + "\n"
                ...
                const std::string& steamUIPath)
{
    ...
    LOG_INFO("SteamDiagnostics: mdx.version={} build={} steamclient64.sha256={} steamui.sha256={}",
             g_snapshot.manifestDexCoreVersion,
             g_snapshot.buildID,
