...
namespace {
    ...
    constexpr const char* kJsdelivrTemplate =
        ...
        "{channel}/{component}/{sha256}.toml";
    constexpr const char* kJsdelivrFastlyTemplate =
        "https://fastly.jsdelivr.net/gh/OpenSteam001/steam-monitor@"
        "{channel}/{component}/{sha256}.toml";
    ...
    static std::vector<std::string> BuildUrlTemplates()
    {
        ...
        if (remoteUrlTemplate.empty())
            return { kGithubTemplate, kJsdelivrTemplate, kJsdelivrFastlyTemplate };
...
Result Fetch(const Request& request)
{
    ...
    fs::path cacheDir  = steamRoot / "manifestdexcore" / request.channel / request.component;
    ...
    // 3. Try remote (mirror chain with fallback on error or 404).
