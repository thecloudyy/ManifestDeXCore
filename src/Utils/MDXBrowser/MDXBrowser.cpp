...
namespace {
// Local-only API keys: NEVER commit real values here — this file is published.
// The Hubcap key is read per-machine from ElementGui settings
// (%AppData%\ElementGui\settings.json -> "HubcapKey"); the settings value wins.
// The Ryuu key is baked in from your own local build environment.
// Empty defaults: both download paths fail gracefully without a key.
constexpr char kRyuuAuthKey[] = "";
constexpr char kHubcapKey[] = "";
