...
bool InitializeSteamComponents()
{
    ...
    sprintf_s(ConfigPath,      kRuntimePathCapacity, "%s\\manifestdexcore.toml", SteamInstallPath);
...
static uint32_t InitThread(OSTPlatform::DynamicLibrary::ModuleHandle selfModule) {
    Log::Init(selfModule);
    LOG_INFO("ManifestDexCore init thread started");
    ...
    // steam-monitor repo. These are embedded in the DLL at build time (see
    // src/pattern/*.supplement.toml), so they survive Steam client updates
    // without a remote round-trip and ship as a single artifact.
    PatternLoader::LoadSupplement(ui_hModule, PatternSupplement::Steamui);
    PatternLoader::LoadSupplement(client_hModule, PatternSupplement::Steamclient);
    ...
    // NOTE: UpdateChecker::Check intentionally not called — the update popup
    // check is disabled; no hash polling, no QueryBox.
    ...
    // Optional store-page button injection (MDXBrowser). No-op unless
    // [mdx_browser].enabled is set.
    MDXBrowser::Initialize(SteamInstallPath);

    LOG_INFO("ManifestDexCore init complete");
    return 0;
}
